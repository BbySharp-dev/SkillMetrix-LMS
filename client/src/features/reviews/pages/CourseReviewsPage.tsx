    ...
    const handleSubmitReview = () => {
        if (newComment.trim()) {
            createReview.mutate(
                ...
                { onSuccess: () => { setNewComment(''); setIsWriting(false); } }
            );
        }
    };
                            ...
                            {[5, 4, 3, 2, 1].map((star) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex items-center gap-2 w-full group ${ratingFilter === star ? 'bg-indigo-50 rounded-lg p-1' : ''}`}
                                    >
            ...
            {(isWriting || userReview) && (
                <Card className="p-6">
                    ...
                    <div className="flex gap-2">
                        <Button onClick={userReview ? handleUpdateReview : handleSubmitReview}>
                            {userReview ? 'Cập nhật' : 'Gửi đánh giá'}
                        </Button>
                        <Button variant="outline" onClick={() => { setIsWriting(false); setNewComment(''); }}>
                            Hủy
                        </Button>
                        {userReview && (
                            <Button
